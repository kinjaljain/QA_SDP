...
def parse(file):
    ...
    for line in data:
        ...
        line_data["Reference Article"] = line_data["Reference Article"].replace(".xml", "").replace(".txt", "").lower().replace("_", "-")
        line_data["Citing Article"] = line_data["Citing Article"].replace(".xml", "").replace(".txt", "").lower().replace("_", "-")
        ...
        global gold
        if gold:
            line_data['Citation Marker Offset'] = line_data["Citation Offset"]
        line_data['Citation Marker Offset'] = line_data['Citation Marker Offset'].replace("'", "").replace(" ", "")
        if line_data["Citation Marker Offset"] not in parse_data[line_data["Reference Article"]][line_data["Citing Article"]]:
            parse_data[line_data["Reference Article"]][line_data["Citing Article"]][line_data["Citation Marker Offset"]] = {"original": line_data, "comparable": True}
        ...
        cnt = 0
        # TODO: Remove this
        ref_text_dict = ref_text_dict["html"]["body"][0]["root"][0]
        for item in ref_text_dict["s"]:
            cnt += 1
        ...
        if len(ref_discourse_facet) > 0:
            if ref_discourse_facet[0] == "[":
                parsed_discourse_facet_tmp = [x.strip().lower().replace(" ", "_").replace("_","") for x in ref_discourse_facet[1:-1].split(",")]
                parsed_discourse_facet = []
                ...
                ref = ref_discourse_facet.lower().replace(" ", "_").replace("_","")
                if ref.startswith("\'") or ref.startswith("\""):
                    ref = ref[1:]
                if ref.endswith("\'") or ref.endswith("\""):
                    ref = ref[:-1]
    ...
    for ref_article in gold_data:
        for cit_article in gold_data[ref_article]:
            for cit_marker_offset in gold_data[ref_article][cit_article]:
                old_TP_ref = TP_ref
                real = gold_data[ref_article][cit_article][cit_marker_offset]["Reference Offset"]
                for ref_offset in gold_data[ref_article][cit_article][cit_marker_offset]["Reference Offset"]:
                    try:
                        ...
                        print("Not found : " + ref_article, cit_article, cit_marker_offset)
            ...
            for cit_marker_offset in gold_data[ref_article][cit_article]:
                for facet in gold_data[ref_article][cit_article][cit_marker_offset]["Discourse Facet"]:
                    if gold_data[ref_article][cit_article][cit_marker_offset]["comparable"]:
                        ...
                        print(facet)
                        if cit_article == "p55-62-p00":
                            cit_article = "p55-62"
                        if cit_article == "w02-0503-parscit130908":
                            cit_article = "w02-0503"
                        if cit_article == "w07-0802-parscit130908":
                            cit_article = "w07-0802"
                        if cit_article == "w10-1730-parscit":
                            cit_article = "w10-1730"
