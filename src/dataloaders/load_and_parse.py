...
def load_article(filename):
    ...
    try:
        ...
        with codecs.open(filename, mode='r', encoding='utf-8', errors='ignore') as target_file:
            return Article(ET.parse(target_file))
    ...
    with open(annotation_file , 'r') as f:
        for line in f:
            ...
            try:
                ...
                cite_article = articles["-".join(parts[2].split(":")[1].strip().upper().replace("_", "-").replace(".XML", "").replace(".TXT", "").split("-")[:2])]
                marker_offset = [int(x) for x in (parts[5].split(":")[1].replace("'", "").replace("[", "").replace("]", "").strip()).replace(",", " ").split()]
                citation_offsets = [int(x) for x in (parts[5].split(":")[1].replace("'", "").replace("[", "").replace("]", "").strip()).replace(","," ").split()]
                ref_offsets = [int(x) for x in (parts[7].split(":")[1].replace("'", "").replace("[", "").replace("]", "").strip()).replace(",", " ").split()]
                facet = parts[9].split(":")[1].strip()
                author = '' if len(parts) < 10 or ":" not in parts[10] else parts[10].split(":")[1].strip()
                d = Datum(123, ref_article, cite_article, Offsets(marker_offset, citation_offsets, ref_offsets), author, is_test, facet)
                data.append(d)
            except Exception as e:
                l.error(e)
...
def load_folder(folder_root):
    ...
    articles = {'-'.join(os.path.splitext(x)[0].upper().replace("_", "-").split("-")[:2]) : load_article(citance_dir + sep + x) for x in os.listdir(citance_dir)}
