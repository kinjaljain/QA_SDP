...
DATA_ROOT = '../../data/Training-Set-2019/Task1'
sep = os.path.sep
...
def get_folders(data_root):
    ...
    for year_folder in filter(lambda x: os.path.isdir(data_root + sep + x), os.listdir(data_root)):
        ...
        for data_folder in os.listdir(year_folder):
            ...
            if not os.path.isdir(data_folder) or 'From-ScisummNet-2019' not in data_folder or 'Test-Set-2018' in data_folder:
                continue
                # readme
    ...
    with open(annotation_file, 'r') as f:
        for line in f:
            ...
            try:
                ...
                #                                                                                             "").strip()
                # cite_article = Article("", ET.fromstring("<xml></xml>"), {}, {})
                ...
                d = Datum(reference, cite, Offsets([],citation_offsets, ref_offsets), author,
                          is_test, facet, year)
                data.append(d)
            except Exception as e:
                l.error(e)
...
def load_folder(folder_root):
    ...
    if os.path.exists(citance_dir):
        articles = {
        '-'.join(os.path.splitext(x)[0].upper().replace("_", "-").split("-")[:2]): load_article(citance_dir + sep + x)
        for x in os.listdir(citance_dir)}
    else:
        # maybe 2019
        json_ = folder_root + sep + "citing_sentences.json"
        if os.path.exists(json_):
            articles = {}
            with open(json_) as f:
                content = json.load(f)
            for paper in content:
                id = paper['citing_paper_id']
                content = paper['raw_text']
                articles[id] = Article("", ET.fromstring("<xml></xml>"), {2019 : content}, {2019 : "N/A"})

        else:
            l.error("Citing articles not loaded")
            articles = {}
...
def load_all(root):
    ...
    for folder in tqdm(folders):
        dataset.append(load_folder(folder))
        i += 1
...
if __name__ == "__main__":
    ...
    meaningless_file = open("meaningless_sentences.txt", "a")
    alphabet_file = open("alphabet_sentences.txt", "a")
    positives_replaced_file = open("positives_replaced.txt", "a")
    ...
    print("dumping")
    import pickle
    with open('processed-data-2019-clean.pkl', 'wb') as f:
